...
() = open_plot("fit_all_lines_voigt1.ps/cps", 1, 2);
fp = fopen("alllinefits_voigt1.txt", "w");


%%%%%%%%%% fit lines in a certain range at a time %%%%%%%%%%%%%%%%
lines = [1.65, 1.85, 1.93, 1.94];
guess_flux = [100, 220, 1000, 800];
lam = [1.4,2];
lam_ignore = [0,1.4];
() = evalfile("fit_line_voigt.sl"); % pass the parameters to "fit_line_voigt.sl", the for loop is in that file.
save_par("voigt1_0.txt");
...
save_par("voigt1_1.txt");

lines = [6.4, 6.7, 6.83, 6.9, 6.94, 7.2];
guess_flux = [400, 200, 80, 300, 200, 50];
lam = [5.5, 7.5];
() = evalfile("fit_line_voigt.sl");
save_par("voigt1_2.txt");
