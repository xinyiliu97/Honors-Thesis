...
flux = np.zeros(shape = (5,12))
fluxerror = np.zeros(shape = (5,12))
x = 0
j = 0
colorls = []
NUM_COLORS = 12
cm = plt.get_cmap('gist_rainbow')
element = ("Mg12", "Si13r", "Si13i", "Si13f", "S15", "Ni27", "Fe25", "Fe26",
	"Mg11r", "Mg11i", "Mg11f", "S16")
MJD = np.array([[58343.62893332, 58343.85713227, 58344.08533122, 58344.31353015, 58344.54172910],]*12).transpose()
print(MJD)
fig, axs = plt.subplots(12,1, figsize=(10,6))
...
for x in range(0,12):
	for j in range(0,5):
		...
		axs[x].errorbar(MJD[j][x], flux[j][x], fluxerror[j][x], marker='s', color = 'red', ms=5, mew=4)
