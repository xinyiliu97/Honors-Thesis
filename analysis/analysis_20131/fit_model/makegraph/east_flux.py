...
element = (r"Fe {\sc xxvi}",r"Fe {\sc xxv}", r"S {\sc xvi}", r"S {\sc xv}", r"Si {\sc xiv}", r"Si {\sc xiii}")
print(element[0])
df = pd.read_csv('east_flux.csv', header=None)
fluxlist = []
fluxerrlist=[]
...
for i in range(6):
	...
	ax = fig.add_subplot(3, 2, i+1) # set the axis for plotting
 	f = flux.values.tolist()      #convert data frame to list
 	fe = fluxerr.values.tolist()
 	elm = element[i]
 	for x in range(6):
		ax.errorbar(MJD[x], f[x], fe[x], marker='o', markersize = 2, color = 'blue',
...
plt.text(-0.5, -1, r" Days since 2018-Aug-10.5076 UTC (MJD 58340.5076)", ha='center', fontsize = 20)			
plt.text(-6, 70, r"Flux ($10^{-6}$ ph cm$^{-2}$ s$^{-1}$)", ha='center', va='center', rotation='vertical', fontsize = 20)
