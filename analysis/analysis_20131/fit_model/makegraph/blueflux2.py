...
MJD = np.zeros((2,5))
fluxshort = (2.16e-5, 6.18e-5)
fluxshorterror = (9.62e-6, 1.128e-5)
fluxshort = [i*1e5 for i in fluxshort]
fluxshorterror = [i*1e5 for i in fluxshorterror]
MJD1 = np.array([[58343.62893332, 58343.85713227, 58344.08533122, 58344.31353015, 58344.54172910],]*2).transpose()
...
color = ("royalblue","grey", "w",  "w", "darkgrey", "w", "w")
...
for x in range(0,2):
	for j in range(0,5):
		...
		if j == 0:
			axs.bar(MJD[j][x] + x*MJDd, flux[j][x], width = MJDd, yerr = fluxerror[j][x], edgecolor="blue", hatch = fill[x], facecolor = color[x], fill = 1, label = element[x])
			axs2.bar(MJD[j][x] + x*MJDd, flux[j][x], width = MJDd, yerr = fluxerror[j][x], edgecolor="blue", hatch = fill[x],facecolor = color[x], fill = 1, label = element[x])
			axs.bar(MJDshortb+x*MJDshortd, fluxshort[x], width = MJDshortd, yerr = fluxshorterror[x], edgecolor="blue", hatch = fill[x], facecolor = color[x], fill = 1)
			axs2.bar(MJDshortb+x*MJDshortd, fluxshort[x], width = MJDshortd, yerr = fluxshorterror[x], edgecolor="blue", hatch = fill[x], facecolor = color[x], fill = 1)
			# fillstyle = "none", markeredgewidth= 1, color = 'red', ms=5, mew=4)

		else:
			axs.bar(MJD[j][x] + x*MJDd, flux[j][x], width = MJDd, 
				yerr = fluxerror[j][x],hatch = fill[x],edgecolor="blue",facecolor = color[x], fill = 1)
			axs2.bar(MJD[j][x] + x*MJDd, flux[j][x],edgecolor="blue", width = MJDd , yerr = fluxerror[j][x], hatch = fill[x],facecolor = color[x], fill = 1)
...
size = 30
fig.text(0.05, .5, r" Line Flux ($10^{-5}$ photons/s/cm$^2$)", ha='center', va='center', rotation='vertical', fontsize = size)
fig.text(0.5, 0.04, r" Days since 2018-Aug-10.0 UTC (MJD 58340.5076)", ha='center', fontsize = size)
...
axs.tick_params(axis='x', which='major', labelsize=size)
axs2.tick_params(axis='x', which='major', labelsize=size)
axs.tick_params(axis='y', which='major', labelsize=size)
axs2.tick_params(axis='y', which='major', labelsize=size)
plt.show()
