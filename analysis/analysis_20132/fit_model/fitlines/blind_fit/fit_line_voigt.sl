...
variable dlam;
% Set gaussian parameters, freezing the centers and sigma
i = 0; while (i < n){
...
i = 0; while (i < n){
  ...
  if (lo_flux == hi_flux ) (lo_flux, hi_flux) = conf(idx, 0);
  flux_sigma = 0.5*(hi_flux-lo_flux);

%conf loop for center wavelength  
  idx = 4*i+5;
  w = _A( get_par(idx) );
  ...
  ok = fprintf (fp, " %3d  %8.4f %8.4f %11.7f %10.7f %8.1f %8.1f\n", i, w,
     wave_sigma, f, flux_sigma, s, width_sigma);
  i = i+1;
}
