...
%; Time-stamp: <2019-06-10 12:30:07 dph>
...
% version 1.3.5 2019.06.10 dph
%  still fixing tgrids in plaw_dem2()


% version 1.3.4 2019.04.28
%  more plaw_dem2() edits...
%  ... since 0.05 dex grid didn't completely succeed; just moved the truncation
%      error in parameter space; diminished a bit in amplitude, but still present
%      especially at higher energies..
%      try to handle the tgrid edges better


% version 1.3.3 2019.04.26
...
private variable _version = [1, 3, 5]   ; % major, minor, patch
...
% Analytic normalizations of powerlaw differential emission measure;
%               ( Tgrid, norm_desired, exponent, T_min, T_max )
%
% NOTE: have some edge effects w/ tgrid; probably need to consider weighting
%    end bins by fractional overlaps with t0, t1.
%    By design, t1 should be a grid-point.

define calc_dem2( tg, tglo, tghi, n, g, t0, t1 )
{
    variable w 	 = Double_Type[ length( tg ) ] ;

    % next line looks wrong:
    %variable l  = where( tglo >= t0 and tghi <= t1); % better grid handling? no...

    % next line sometimes gives wrong grid:  [1.3.4];
    %variable l 	 = where( tg > t0 and tg <= t1);

    % v 1.3.5: try better grid edges:  inclusive of tglo, tghi
    %   (but still not weighting end bins by fractional values):
    variable l 	 = where( tghi >= t0  and tglo <= t1 ) ; 

    variable gp1 = g + 1.0 ;  % gamma + 1 
    variable k ;

    if ( g != -1 )  k =   gp1 /   t1 / ( 1.0 - (t0/t1)^gp1 ) ;
    else	    k =  1.0  / ( t1 * log( t1 / t0 ) ) ;
...
define mk_tgrid( t0dex, t1dex )
{
    ...
    % make sure tg[-1] is exactly t1dex (which can be more important than the 1st grid point):
    tg *= (10^t1dex / tg[-1]);

    % Custom Aped_emdx model expects a 2*26 T grid.
...
public define plaw_dem2_fit( lo, hi, par )
{
    ...
    % 2019.02.20 dph
    % modify: using the default emd grid introduces discretization in
    % the temperature parameters, which showed strongly in conf
    % maps. To avoid this, define a local, custom grid from T0 to T1
    % at approximately 0.05 dex steps.
    %
    variable tg, dlnT ;
    ( tg, dlnT ) = mk_tgrid( T0_dex, T1_dex );
    variable tglo = tg / exp(dlnT/2 );
    variable tghi = tg * exp(dlnT/2 );

    variable a = calc_dem2( tg, tglo, tghi, norm, gamma, T0, T1 );
    variable w = a * tg * dlnT ;    % weights log integral over uniform dlnT bins
...
define plaw_dem2_par_to_plasma_state( Par )
{
    ...
    variable a = calc_dem2( emd_t_mid, emd_t_lo, emd_t_hi, norm, gamma, T0, T1 );
